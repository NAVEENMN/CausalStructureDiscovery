...
data_observations_path = os.path.join(args.path, f'observations_{args.exp}.csv')
springs_observations_path = os.path.join(args.path, f'springs_{args.exp}.csv')
...
def get_auroc(time_step, p_values_dim_1, p_values_dim_2, _springs):
    ...
    details = f'p_threshold: {args.pv}, taumin: {experiment.get_tau_min()},, taumax: {experiment.get_tau_max()}, fpr: {round(np.mean(fpr), 2)}, tpr: {round(np.mean(tpr), 2)}, auroc:{round(auroc, 2)}'
...
def save_graph(time_step, causal_graph, _variables, details):
    ...
    s_mat = []
    for _var in spring_vars:
        s_mat.append(_springs.iloc[time_step][_var])
    s_mat = np.reshape(s_mat, (int(len(spring_vars)/2), int(len(spring_vars)/2)))
...
def run_pc(dim):
    parents = get_parents(tau_min=experiment.get_tau_min(), tau_max=experiment.get_tau_max())
    if dim == 1:
    ...
    pcmci.verbosity = 0
    results = pcmci.run_pcmci(tau_max=experiment.get_tau_max(),
                              tau_min=experiment.get_tau_min(),
...
def main():
    ...
    # *** Control Variables ***
    tau_max = experiment.get_tau_max()
    p_threshold = args.pv
